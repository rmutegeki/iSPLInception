function varargout = label_panorama_2009(varargin)
% LABEL_PANORAMA_2009 MATLAB code for label_panorama_2009.fig
%      LABEL_PANORAMA_2009, by itself, creates a new LABEL_PANORAMA_2009 or raises the existing
%      singleton*.
%
%      H = LABEL_PANORAMA_2009 returns the handle to a new LABEL_PANORAMA_2009 or the handle to
%      the existing singleton*.
%
%      LABEL_PANORAMA_2009('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LABEL_PANORAMA_2009.M with the given input arguments.
%
%      LABEL_PANORAMA_2009('Property','Value',...) creates a new LABEL_PANORAMA_2009 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before label_panorama_2009_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to label_panorama_2009_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help label_panorama_2009

% Last Modified by GUIDE v2.5 07-Jun-2012 13:49:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @label_panorama_2009_OpeningFcn, ...
                   'gui_OutputFcn',  @label_panorama_2009_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
for c = 1:length(dataset.label_columns)
    ...
    dataset.label_id2name{c} = containers.Map(1.1,'a');     % Hack to set container type in older matlabs
    dataset.label_id2name{c}.remove(1.1);                   % Hack to set container type in older matlabs
    ...
    dataset.label_id2img_coord{c} = containers.Map(dataset.label_id2name{c}.keys, num2cell(1:length(dataset.label_id2name{c}.keys)));
    %dataset.label_img_coord2id{c} = containers.Map(1:length(keys(dataset.label_id2name{c})), keys(dataset.label_id2name{c}));
    dataset.label_img_coord2id{c} = containers.Map(num2cell(1:length(dataset.label_id2name{c}.keys)), dataset.label_id2name{c}.keys);
    
    %dataset.label_image{c} = zeros(1,length(keys(dataset.label_id2img_coord{c})));
    dataset.label_image{c} = zeros(1,length(dataset.label_id2img_coord{c}.keys));
...
set(gca, 'XTickLabel', handles.dataset.label_id2name{column}.keys);
...
% --- Executes just before label_panorama_2009 is made visible.
function label_panorama_2009_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to label_panorama_2009 (see VARARGIN)

% Choose default command line output for label_panorama_2009
...
% UIWAIT makes label_panorama_2009 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = label_panorama_2009_OutputFcn(hObject, eventdata, handles) 
